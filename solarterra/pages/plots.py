...
def get_query_params(variable):
    ...
    q_params = {
        ...
        # TODO there's DataType model now, refactor
        'model_class': variable.dataset.dynamic.resolve_class(),
    }
...
def get_plot(qs, p, q_params):
    ...
    for field in q_params['list_of_fieldnames']:
        ...
        _id1, _pos1, g_count_1 = np.unique(
            groups_1[_ndx_1], return_index=True, return_counts=True)
    ...
    complete_df['dt_timestamp'] = complete_df.apply(
        lambda x: its(x[time_field]), axis=1)
...
def get_plots(variables, ts_limits, ts_limits_dt):
    ...
    plot_params['bin_size'], plot_params['new_ts_limits'] = get_bin_size(
        ts_limits)
    plot_params['bin_size_dt'] = dt.timedelta(seconds=plot_params['bin_size'])
    plot_params['hbs'] = math.ceil(plot_params['bin_size'] / 2)
    plot_params['t'] = np.arange(plot_params['new_ts_limits'][0],
                                 plot_params['new_ts_limits'][1], step=plot_params['bin_size'])
    plot_params['t_df_index'] = np.arange(1, plot_params['t'].size)
    plot_params['tm'] = np.arange(
        plot_params['t'][0] + plot_params['hbs'], plot_params['t'][-1], step=plot_params['bin_size'])
