...
def value_arrays(dynamic_fields, cdf_object):
    ...
    for field in dynamic_fields:
        ...
        # if array of floats
            ...
            #just in case for multiple values
            # for fv in fill_val:
            #     b[b == fv] = math.nan
            ...
            #digits_after_point = Decimal(str(10 ** -(places)))
            ...
            for index, x in enumerate(b):
                if x == str_nan:
                    ...
                    #ll.append(float(x))
                    ll.append(round(float(x),digits_after_point)) #❓❓ not sure if i should round it here, but um
                    print(x, places, digits_after_point)

                    #item = Decimal(x).quantize(digits_after_point, context=context)
                    #ll.append(item)
        ...
        # any other array
            ...
            fill_val, fill_val_type = var.fillval, var.datatype #generally fillval type matches with var datatype
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        files_list = []
        #TODO: should extract only files from current upload 
