...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        #     # zip_md5=zip_md5
        # )
        uploads = Upload.objects.filter(u_tag=upload_u_tag)

        uploads.delete()
        # to truncate logfile fast
        with open(settings.LOG_FILE, mode="w"):
            pass
        ...
        collision_logs_txt = os.path.join(
            settings.COLLISIONS_LOGS, f"{dataset_tag}_{upload_u_tag}_collisions.txt")
        if os.path.exists(collision_logs_txt):
            os.remove(collision_logs_txt)

        cdf_files = [f for f in os.listdir(dataset_dir) if f.endswith('.cdf')]
        ...
        #             collision_logs_txt = os.path.join(
        #                 COLLISION_LOGS_DIR, f"{dataset_tag}_{upload.u_tag}_collisions.txt")
